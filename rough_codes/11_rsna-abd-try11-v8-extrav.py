...
class CFG:
    ...
    OUTPUT_FOLDER = f"/mnt/md0/rsna_abd/AAA_CLS/TRY11_CLS/{model_name}_v{V}"
...
data = pd.read_csv('/mnt/md0/rsna_abd/info_data1_processed_v2.csv')
...
class AbdDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        vol = np.load(f"/mnt/md0/rsna_abd/data_theo/{patient}_{study}_{start}_{end}.npy")
...
def get_metric_score(keys, NO):
    ...
    valid_study_level_pred['extravasation_injury'] = (study_level['extravasation_injury'].mean() * 3) + (NO[:, 0]) * 4
    valid_study_level_pred['extravasation_healthy'] = study_level['extravasation_healthy'].mean() * 0.5 + (1-NO[:, 0]) * 0.5
